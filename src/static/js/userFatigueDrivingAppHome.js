...
window.addEventListener('message', event => {
    ...
    // 获取应用文件列表
    if (message.cmd == 'getUserFatigueDrivingAppsListRet') {
...
new Vue({
    ...
    mounted() {
        callbacks('getUserFatigueDrivingAppsList', userAppsList => {
    ...
    methods: {
        gotoOneUserFatigueDrivingAppPage(id) {
            console.log("run app: ", id);
            // 给插件发送消息 跳转到用户应用页面
            vscode.postMessage({
                command: 'gotoOneUserFatigueDrivingAppPage',
                text: id,
            });
        },
    } 
