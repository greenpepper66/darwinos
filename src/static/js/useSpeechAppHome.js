...
window.addEventListener('message', event => {
    ...
    // 获取应用文件列表
    if (message.cmd == 'getUserSpeechAppsListRet') {
...
new Vue({
    ...
    mounted() {
        callbacks('getUserSpeechAppsList', userAppsList => {
    ...
    methods: {
        gotoOneUserSpeechAppPage(id) {
            console.log("run speech app: ", id);
            // 给插件发送消息 跳转到用户应用页面
            vscode.postMessage({
                command: 'gotoOneUserSpeechAppPage',
                text: id,
            });
        },
    } 
