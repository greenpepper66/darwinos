...
window.addEventListener('message', event => {
    ...
    // 获取应用文件列表
    if (message.cmd == 'getUserAgeJudgeAppsListRet') {
        this.userAppsList = message.data;
        console.log(message.data);
        (callbacks[message.cbid] || function () { })(message.data);
        delete callbacks[message.cbid];
        console.log('---------------------------message：get user age judge app list', this.userAppsList.length);
    }
...
new Vue({
    ...
    mounted() {
        callbacks('getUserAgeJudgeAppsList', userAppsList => {
    ...
    methods: {
        gotoOneUserAgeJudgeAppPage(id) {
            console.log("run age judge app: ", id);
            // 给插件发送消息 跳转到用户应用页面
            vscode.postMessage({
                command: 'gotoOneUserAgeJudgeAppPage',
                text: id,
            });
        },
    } 
