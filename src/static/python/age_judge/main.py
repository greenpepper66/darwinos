...
inputImgFile = sys.argv[1]  # 用户上传的年龄检测照片文件
configDir = sys.argv[2]  # 解包后的配置文件路径
outputDir = sys.argv[3]  # 生成的编码文件所在路径 


print(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'), "start age-judge img encode.")
...
label_txts = ['(0, 20)', '(21, 32)', '(33, 48)', '(49, 59)', '(60, 100)'] # 年龄段
