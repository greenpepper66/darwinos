...
function compress(data, inputSampleRate, outputSampleRate) {
    // 压缩，根据采样率进行压缩
    var rate = inputSampleRate / outputSampleRate, compression = Math.max(rate, 1), lData = data.left, rData = data.right, length = Math.floor((lData.length + rData.length) / rate), result = new Float32Array(length), index = 0, j = 0;
 ...
 */
function encodePCM(bytes, sampleBits, littleEdian) {
...
function encodeWAV(bytes, inputSampleRate, outputSampleRate, numChannels, oututSampleBits, littleEdian) {
    if (littleEdian === void 0) { littleEdian = true; }
    var sampleRate = outputSampleRate > inputSampleRate ? inputSampleRate : outputSampleRate, // 输出采样率较大时，仍使用输入的值，
