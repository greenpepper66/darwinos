...
function writeImgAppInfoToJson(global, message, appType) {
    if (message.text.length != 5 || message.text[0] == "" || message.text[1] == "" || message.text[2] == ""
        ...
        // 检查名字是否重复
        if (searchImgAppByName(global.context, message.text[0], appType) != "none") {
            console.log("应用名字重复");
            ...
            if (modelInfo.length != 4) {
                ...
                imgAppConfig.appType = appType;  // 应用类型： 0-手写体图像识别， 1-疲劳检测, 2-语音识别
 ...
 */
// 1. 语音识别应用列表首页交互
const speechAppMessageHandler = {
    // 1.1 打开新建语音应用页面
    gotoNewSpeechAppPage(global, message) {
        console.log(message);
        openNewSpeechAppPage(global.context);
    },

    // 1.2 查询所有应用列表
    getSpeechAppsConfigList(global, message) {
        console.log(message);
        let allApps = searchAllJson(global.context, 2);
        global.panel.webview.postMessage({ cmd: 'getSpeechAppsConfigListRet', cbid: message.cbid, data: allApps });
    },

    // 2.3 删除应用
    deleteSpeechAppConfig(global, message) {
        console.log(message);
        let delRet = deleteJson(global.context, message.text, 2);
        console.log("deleteSpeechAppConfig ret: ", delRet);
        global.panel.webview.postMessage({ deleteSpeechAppConfigRet: message.text });
    },

    // 2.4 查询应用  显示详情页
    gotoSpeechAppInfoPage(global, message) {
        console.log(message);
        openSpeechAppInfoPage(global.context, message.text);
    },
};

// 2. 打开语音识别应用首页, 最近应用列表页面
export function openSpeechAppHomePage(context) {
    console.log("IDE openSpeechAppHomePage!", IDEPanels.speechAppListPanel);
    if (IDEPanels.speechAppListPanel) {
        console.log("打开语音识别应用列表页面：", IDEPanels.speechAppListPanel.visible);
        IDEPanels.speechAppListPanel.reveal();
    } else {
        console.log("新建语音识别应用列表页面");
        IDEPanels.speechAppListPanel = vscode.window.createWebviewPanel(
            'speechAppListPage',
            "语音识别",
            vscode.ViewColumn.One,
            {
        ...
        let panel = IDEPanels.speechAppListPanel;
        let global = { panel, context };

        IDEPanels.speechAppListPanel.webview.html = getAppsHomeHtml(context, speechAppHomeHtmlFilePath);
        IDEPanels.speechAppListPanel.webview.onDidReceiveMessage(message => {
            if (speechAppMessageHandler[message.command]) {
                speechAppMessageHandler[message.command](global, message);
            } else {
                vscode.window.showInformationMessage(`未找到名为 ${message.command} 回调方法!`);
            }
        }, undefined, context.subscriptions);

        console.log("IDE openSpeechAppHomePage 2!", global.panel);

        // 面板被关闭后重置
        IDEPanels.speechAppListPanel.onDidDispose(
            () => {
                IDEPanels.speechAppListPanel = undefined;
            },
            null,
            context.subscriptions
        );
...
// 3. 新建语音应用页面交互
const newSpeechAppMessageHandler = {
    // 1.1 获取模型文件
    getSpeechModelFileList(global, message) {
        console.log(message);
        console.log("models list:", allData.deployedModelList);
        global.panel.webview.postMessage({ getSpeechModelFileListRet: allData.deployedModelList });
    },

    // 1.2 选择编码所需的各个配置文件 - 打包成一个了
    selectSpeechEncodeConfFile(global, message) {
        console.log(message);
        const options: vscode.OpenDialogOptions = {
            openLabel: "选择文件",
            // canSelectFolders: true,
        ...
        vscode.window.showOpenDialog(options).then(fileUri => {
            console.log("选择配置文件为", fileUri);
            global.panel.webview.postMessage({ selectSpeechEncodeConfFileRet: fileUri[0].fsPath });
        });
    },

    //1.3 选择编码过程中输出文件所在的文件夹
    selectSpeechOutputDir(global, message) {
        console.log(message);
        const options: vscode.OpenDialogOptions = {
            openLabel: "选择目录",
            canSelectFolders: true,
        ...
        vscode.window.showOpenDialog(options).then(fileUri => {
            console.log("选择目录为", fileUri);
            global.panel.webview.postMessage({ selectSpeechOutputDirRet: fileUri[0].fsPath });
        });
    },

    // 1.4 保存应用配置 - 新建应用页面的“保存应用”按钮触发
    saveSpeechAppConfig(global, message) {
        console.log(message);
        let saveRet = writeImgAppInfoToJson(global, message, 2);
        console.log("saveSpeechAppConfig save result： ", saveRet);
        global.panel.webview.postMessage({ saveSpeechAppConfigRet: saveRet });
    },

    // 1.5 应用保存成功后，弹出框显示“保存成功！”, 页面跳转到应用列表页面
    newSpeechAppSaveSuccGotoListPage(global, message) {
        console.log(message);
...
// 4. 打开新建语音应用页面
export function openNewSpeechAppPage(context) {
    console.log("IDE openNewSpeechAppPage!", IDEPanels.newSpeechAppPanel);
    if (IDEPanels.newSpeechAppPanel) {
        console.log("打开新建语音识别应用页面：", IDEPanels.newSpeechAppPanel.visible);
        IDEPanels.newSpeechAppPanel.reveal();
    } else {
        console.log("新建 新建语音识别应用页面");

        IDEPanels.newSpeechAppPanel = vscode.window.createWebviewPanel(
            'newSpeechAppPage',
            "新建语音识别应用",
            vscode.ViewColumn.One,
            {
        ...
        let panel = IDEPanels.newSpeechAppPanel;
        let global = { panel, context };
        IDEPanels.newSpeechAppPanel.webview.html = getAppsHomeHtml(context, newSpeechAppHtmlFilePath);

        IDEPanels.newSpeechAppPanel.webview.onDidReceiveMessage(message => {
            if (newSpeechAppMessageHandler[message.command]) {
                newSpeechAppMessageHandler[message.command](global, message);
            } else {
                vscode.window.showInformationMessage(`未找到名为 ${message.command} 回调方法!`);
            }
        }, undefined, context.subscriptions);

        console.log("IDE openNewSpeechAppPage 2!", global.panel);

        // 面板被关闭后重置
        IDEPanels.newSpeechAppPanel.onDidDispose(
            () => {
                IDEPanels.newSpeechAppPanel = undefined;
            },
            null,
            context.subscriptions
        );
...
// 5. 应用详情页面交互
const speechAppInfoMessageHandler = {
    getSpeechAppInfos(global, message) {
        console.log(message);
        let infos = searchImgAppByID(global.context, global.appID, 2);
        global.panel.webview.postMessage({ cmd: 'getSpeechAppInfosRet', cbid: message.cbid, data: infos });
    },
};

// 6. 打开应用详情页面
export function openSpeechAppInfoPage(context, appID) {
    console.log("IDE openSpeechAppInfoPage!", IDEPanels.speechAppInfoPagePanelsMap);
    if (IDEPanels.speechAppInfoPagePanelsMap.has(appID)) {
        console.log("打开语音识别应用详情页面：", IDEPanels.speechAppInfoPagePanelsMap.get(appID).visible);
        IDEPanels.speechAppInfoPagePanelsMap.get(appID).reveal();
    } else {
        console.log("新建语音识别应用详情页面");
        let panel = vscode.window.createWebviewPanel(
            'speechAppInfoPage',
            "应用详情",
            vscode.ViewColumn.One,
        ...
        panel.webview.html = getAppsHomeHtml(context, speechAppInfoHtmlFilePath);

        // 保存应用ID
        let global = { panel, context, appID };
        IDEPanels.speechAppInfoPagePanelsMap.set(appID, panel);

        panel.webview.onDidReceiveMessage(message => {
            if (speechAppInfoMessageHandler[message.command]) {
                speechAppInfoMessageHandler[message.command](global, message);
            } else {
                vscode.window.showInformationMessage(`未找到名为 ${message.command} 回调方法!`);
            }
        }, undefined, context.subscriptions);

        console.log("IDE openSpeechAppInfoPage 2!", global.panel);

        // 面板被关闭后重置
        panel.onDidDispose(
            () => {
                panel = undefined;
                IDEPanels.speechAppInfoPagePanelsMap.delete(appID);
            },
            null,
            context.subscriptions
        );
